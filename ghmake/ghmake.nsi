...
OutFile "ghmake_installer.exe"

InstallDir $PROGRAMFILES\GeneHuggers\ghmake

InstallDirRegKey HKLM "Software\GeneHuggers\ghmake" "Install_Dir"
...
SectionGroup	"Dependencies"
	...
	Section "-"
		IfFileExists "$INSTDIR\ghmake.exe" 0 new_install
...
;	SectionIn RO
	...
	File "release\ghmake.exe"

	WriteRegStr HKLM SOFTWARE\GeneHuggers\ghmake "Install_Dir" "$INSTDIR"
...
Section "Start Menu Shortcuts"
	...
	CreateShortCut "$SMPROGRAMS\GeneHuggers\ghmake.lnk" \
	"$INSTDIR\ghmake.exe" "" "$INSTDIR\ghmake.exe" 0
	CreateShortCut "$SMPROGRAMS\GeneHuggers\ghmake\Uninstall.lnk" \
...
Section "Uninstall"
	...
	DeleteRegKey HKLM \
	"Software\Microsoft\Windows\CurrentVersion\Uninstall\GeneHuggers"
	DeleteRegKey HKLM SOFTWARE\GeneHuggers\ghmake

	Delete $INSTDIR\ghmake.exe
	Delete $INSTDIR\uninstall.exe

	Delete "$SMPROGRAMS\GeneHuggers\ghmake\*.*"

	RMDir "$SMPROGRAMS\GeneHuggers\ghmake"
	RMDir "$INSTDIR"
SectionEnd
