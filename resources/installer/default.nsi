...
OutFile "GH_APP_VAR_installer.exe"

InstallDir $PROGRAMFILES\GeneHuggers\GH_APP_VAR

InstallDirRegKey HKLM "Software\GeneHuggers\GH_APP_VAR" "Install_Dir"
...
!define	GH_VER "5.0.05"
!define	GH_DLL '${GH_VER}.dll'
;!define	GH_LIBS "GH_PATH_VAR"
!define	GH_LIBS "C:\Gh"
...
;	Section "-"
;		IfFileExists "$INSTDIR\GH_APP_VAR.exe" 0 new_install
...
;	SectionIn RO
	...
	File "release\GH_APP_VAR.exe"

	WriteRegStr HKLM SOFTWARE\GeneHuggers\GH_APP_VAR "Install_Dir" "$INSTDIR"
...
Section "Start Menu Shortcuts"
	...
	CreateShortCut "$SMPROGRAMS\GeneHuggers\GH_APP_VAR.lnk" \
	"$INSTDIR\GH_APP_VAR.exe" "" "$INSTDIR\GH_APP_VAR.exe" 0
...
;	!insertmacro UninstallLib REGDLL SHARED NOREBOOT_PROTECTED $SYSDIR\${QODBC_DLL}
	...
	DeleteRegKey HKLM \
	"Software\Microsoft\Windows\CurrentVersion\Uninstall\GeneHuggers"
	DeleteRegKey HKLM SOFTWARE\GeneHuggers\GH_APP_VAR

	Delete $INSTDIR\GH_APP_VAR.exe
	Delete $INSTDIR\uninstall.exe

	Delete "$SMPROGRAMS\GeneHuggers\GH_APP_VAR\*.*"

	RMDir "$SMPROGRAMS\GeneHuggers\GH_APP_VAR"
	RMDir "$INSTDIR"
SectionEnd
